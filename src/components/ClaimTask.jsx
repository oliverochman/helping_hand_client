    ...
    claimButton = (
      <Grid>
        <Grid.Column align='center'>
          <Button id='create-request' onClick={() => getConfirmedTasks(dispatch)}>
            Offer Help
          </Button>
        </Grid.Column>
      </Grid>
      ...
      return (
        <>
          ...
          ><List.Content className='taskOwner' key={`task-${task.id}-user`} id={`task-${task.id}-user`}>
            ...
            <Button id={`task${task.id}-button`} key={`task-${task.id}-button`} onClick={(e) => claimTask(e, dispatch)}>Claim Task</Button>
          </List.Item>
        </>
      )
    })
